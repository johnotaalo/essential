...
class M_complete_survey extends MY_Model
{
    ...
    private function addQuestionsInfo() {
        ...
        for ($i = 1; $i <= $this->noOfInsertsBatch; ++$i) {
            ...
            (array_key_exists('questionReasonOther', $this->elements[$i]) && $this->elements[$i]['questionReasonOther'] != '') ? $this->theForm->setLqReason($this->elements[$i]['questionReasonOther']) : $x = 1;
            (array_key_exists('questionSpecified', $this->elements[$i]) && $this->elements[$i]['questionSpecified'] != '') ? $this->theForm->setLqSpecifiedOrFollowUp($this->elements[$i]['questionSpecified']) : $this->theForm->setLqSpecifiedOrFollowUp('n/a');
            (array_key_exists('questionSpecifiedOther', $this->elements[$i]) && $this->elements[$i]['questionSpecifiedOther'] != '') ? $this->theForm->setLqSpecifiedOrFollowUp($this->elements[$i]['questionSpecifiedOther']) : $x = 1;
    ...
    private function addMchStaffTrainingInfo() {
        ...
        for ($i = 1; $i <= $this->noOfInsertsBatch + 1; ++$i) {
            ...
            $this->theForm = $this->getStoredData('models\Entities\TrainingGuidelinesN', array('ssId' => $this->session->userdata('survey_status'), 'tgStaff' => $this->elements[$i]['mchTrainingStaff'],'guideCode' => $this->elements[$i]['mchGuideline']));
