      ...
      success: function(data) {
        ...
        if (obj.chart_series != null && obj.chart_series[0] != null) {
          $(graph_section).append('<div id="' + obj.container +
            '" ></div>');
          $(graph_section).parent().parent().find(
            '.portlet-title h6 .sizer').attr('data-url', function_url);
          $(graph_section).parent().parent().find(
            '.portlet-title h6 .sizer').attr('data-for', obj.data_for);
          $(graph_section).parent().parent().find(
            '.portlet-title h6 .sizer').attr('data-parent', obj.data_parent);
          $(graph_section).parent().parent().find(
            '.portlet-title h6 .sizer').attr('data-statistic', obj.statistics);
          graph_text = $(graph_section).parent().parent().find(
            '.portlet-title h6 .graph-title').text();
          $(graph_section).parent().parent().find(
            '.portlet-title h6 .sizer').attr('data-text', graph_text);
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $.each(obj, function(k, v) {
        ...
        phoneNumber = encodeURIComponent(getCountryInfo(v.cl_country) +
          v.cl_phone_number);
...
function notify_sms(phoneNumber, message) {
    ...
    $.ajax({
      //url: base_url + function_url,
      url: 'http://localhost/mnh/c_admin/notify/sms/' + phoneNumber + '/' +
...
function notify_email(email, message) {
  ...
  $.ajax({
    //url: base_url + function_url,
    url: 'http://localhost/mnh/c_admin/notify/email/' + email + '/' +
      encodeURIComponent(message),
    beforeSend: function(xhr) {
      xhr.overrideMimeType("text/plain; charset=x-user-defined");
    },
...
function getCountyData(base_url, county, survey_type, survey_category) {
  ...
  $.ajax({
    url: base_url + 'analytics/getCountyData/' + survey_type + '/' +
    ...
    success: function(data) {
      ...
      percentage = Math.round((obj[0].reported / obj[0].actual * 100),
        2);
      $('#county_progress .progress-bar').text(percentage + '%');
      $('#county_progress .progress-bar').attr('aria-valuenow',
        percentage);
      $('#county_progress .progress-bar').css('width', percentage + '%');
      url = base_url + 'analytics/setActive/' + county + '/' +
        survey_type + '/' + survey_category;
      $('#load_analytics').attr('data-url', url);
      new_url = base_url + 'analytics/getCountyReportingSummary/' +
        county + '/' + survey + '/' + survey_category;
...
function submitForm(base_url, function_url) {
   ...
   * @return {[type]}              [description]
   */
function loadModalForm(base_url, function_url, modal_title, modal_width,
    contents) {
    ...
    $('#universalModal .content').append(contents['graph'] + contents[
      'facility_data']);
...
function loadHelpForm(base_url) {
  helpform =
    '<form id="modal-form" method="post"> <label>Email Address</label> <input type="email" placeholder="Please Enter Email Address Here...">' +
...
$(document).ready(function() {
  ...
  $(document).on('datatable_loaded', function() {
    ...
    // paginate.remove();
    // paginate.addClass('ui pagination menu');
    // paginate.find('a').addClass('item');
    // $('#DataTables_Table_0_info').append(paginate);
